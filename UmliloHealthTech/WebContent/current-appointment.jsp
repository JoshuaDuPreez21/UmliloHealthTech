...
<body>
	<aside class="sidebar" aria-label="Nurse workspace menu">
		...
		<nav class="nav-menu">
			...
			<a href="current-appointment" class="nav-item-link active"><i class="bi bi-activity"></i><span class="nav-label">Visits Today</span></a>
...
<script>
	...
	function normalizeVisit(item) {
		...
		var status = titleCase(item.status || "Waiting");
		var visitType = getSummaryField(summary, "Visit Type") || "Walk-in";
		var department = getSummaryField(summary, "Department") || "General";
		var provider = getSummaryField(summary, "Attending Provider") || item.doctorName || "Provider not assigned";
		var complaint = getSummaryField(summary, "Chief Complaint") || "Clinical visit";
		var details = getSummaryField(summary, "Additional Details");
		return {
			id: item.id,
			patientName: item.patientName || "Unnamed Patient",
			dateTime: item.dateTime || "-",
			status: status === "New" ? "Waiting" : status,
			visitType: visitType,
			department: department,
			provider: provider,
			complaint: complaint,
			diagnosis: details || summary || "Assessment pending"
		};
	}
	...
	function statusText(status) {
		if (status === "Pending Doctor") return "Triage";
	...
	function updateFilterOptions(visits) {
		...
		var statuses = Array.from(new Set(visits.map(function (visit) { return statusText(visit.status); }))).sort();
	...
	function renderVisits() {
		...
		var filtered = allVisits.filter(function (visit) {
			return (!statusValue || statusText(visit.status) === statusValue) &&
		...
		list.innerHTML = "";
		if (!filtered.length) {
			list.innerHTML = "<div class=\"empty-state\">No visit records match the selected filters.</div>";
			return;
		}

		filtered.forEach(function (visit) {
			var status = statusText(visit.status);
